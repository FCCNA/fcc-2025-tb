...
def is_saturato(wf, bit = 14):
    threshold = 16381
    return (np.max(wf) >= threshold or np.min(wf) <= 2)
...
def read_waveform(run_index, path, om, fc, json_data, save_waveforms):
    ...
    for event in mfile:
        ...
        for bank_name, bank in event.banks.items():
            ...
            if is_saturato(bank.data, bit = bit):
                satur[name] += 1
    ...
    if not fc:
        ...
        if om:
            list_of_cuts_plastic = {
                'Trigger': None,
                'Plastico': output_df[('Plastico', 'amplitude')] > 0.7,
                'Chambers': (
            ...
            create_efficiency_plot(output_df, list_of_cuts_plastic, run_index, paths, 'plastic')
    ...
    if om:
        ...
        if 'Chamber_x_1' in output_df.columns.get_level_values(0) and 'Chamber_x_2' in output_df.columns.get_level_values(0) and \
           'Chamber_y_1' in output_df.columns.get_level_values(0) and 'Chamber_y_2' in output_df.columns.get_level_values(0):
            ...
            output_df_filtered_plastico = output_df_filtered[output_df_filtered[('Plastico', 'amplitude')] > 0.7]
        ...
        if not fc:
            ...
            if other_channels:
                ...
                for idx, name in enumerate(other_channels):
                    if (name, 'amplitude') in output_df_filtered_plastico.columns:
                        axes[idx].hist(output_df_filtered_plastico[(name, 'amplitude')], bins=100, alpha=0.7)
